...
export async function POST(req: NextRequest) {
    ...
    const { full_name, dob, document_type, document_number, document_image_url } = await req.json()

    await pool.query(
        `INSERT INTO kyc (user_id, full_name, dob, document_type, document_number, document_image_url, status)
     ...
     ON CONFLICT (user_id) DO UPDATE SET 
       ...
       status = 'pending'`,
        [user.id, full_name, dob, document_type, document_number, document_image_url]
    )
