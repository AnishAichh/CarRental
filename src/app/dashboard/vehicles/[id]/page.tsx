...
export default function VehicleDetailPage() {
    ...
    const handleBooking = async (e: React.FormEvent) => {
        ...
        if (!user.is_kyc_verified || user.kyc_status !== 'approved') {
            setError('You must complete KYC verification before booking')
            router.push('/dashboard/kyc?redirect=' + encodeURIComponent(`/dashboard/vehicles/${vehicleId}`))
            return
        }
        ...
        try {
            ...
            const totalAmount = (vehicle?.price_per_day || 0) * days
            const payload = {
            ...
            if (!res.ok) {
                if (data.error === 'Vehicle is not available for the selected dates') {
            ...
            {error && (
                <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4 relative">
                    ...
                    {user && !user.is_kyc_verified && (
                        <div className="mt-2">
                            <Link
                                href={`/dashboard/kyc?redirect=${encodeURIComponent(`/dashboard/vehicles/${vehicleId}`)}`}
                                className="text-blue-600 hover:underline"
                            >
